...
async def run(root: Path, question: str, model: str, *,
              ...
              num_predict: int = 400, on_step=None) -> Run:
    """Drive the MCP tools with `model` until it answers.
    ...
    async with stdio_client(params) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            for rnd in range(1, max_rounds + 1):
                out.rounds = rnd
                msg = _chat(model, messages, tools, num_ctx, num_predict)
                ...
                for call in calls:
                    ...
                    try:
                        ...
                        # Feed the failure back as a tool result rather than
                        # aborting: the model can pick a different tool.
